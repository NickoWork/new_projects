...
void Post_menu();
void Gleb();
int HashCheck(int currentHashKey);
int GetHashSum();
...
void Post_menu() {
  ...
  switch (pick) {
    case 1:
      ...
      break;
    case 2:
      snprintf(str, len, "2.txt");
      Start_init_1(str);
      break;
    case 3:
      snprintf(str, len, "3.txt");
      Start_init_1(str);
      break;
    case 4:
      snprintf(str, len, "4.txt");
      Start_init_1(str);
      break;
    case 5:
      snprintf(str, len, "5.txt");
      Start_init_1(str);
      break;
    case 0:
      system("clear");
...
void Ispolnyaet() {
  printf("Enter the speed 1-10 or more >> ");
  scanf("%d", &speed);
  if (speed == 0) {
    printf("Error!!!");
    Menu();
...
void Gleb() {
  ...
  FILE *fp;
  char str[128];
  if ((fp = fopen("ascii-art.txt", "r")) == NULL) {
...
int GetHashKey(int countI, int countJ, int countN) {
  const int keyI = 103, keyJ = 371, keyN = 97;
  return ((countI * keyI) + (countJ * keyJ) + (countN * keyN));
}
int GetHashSum() {
  int countx = 0, county = 0, countN = 0;
  for (int y = 1; y < HEIGHT - 1; y++) {
    for (int x = 1; x < LENGTH - 1; x++) {
      if (gen[x][y] == '1') {
        countx += x;
  ...
  return GetHashKey(countx, county, countN);
}

int HashCheck(int currentHashKey) {
  int isRepeat = 0;
  for (int i = 0; i < 100; i++) {
    if (hash[i] == currentHashKey) isRepeat++;
  }
  if (isRepeat == 0) {
    for (int j = 99; j > 0; j--) {
      hash[(j - 1)] = hash[j];
    }
    hash[99] = currentHashKey;
  }
  return isRepeat;
}
